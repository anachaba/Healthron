...
def MASK_DETECTION(frame, faceNet, maskNet):
    ...
    for i in range(0, detections.shape[2]):
        ...
        if confidence > 0.4:
            ...
            # lists
            MASK_DETECTION.faces.append(face)
            locs.append((startX, startY, endX, endY))

    # only make a predictions if at least one face was detected
    if len(MASK_DETECTION.faces) > 0:
        MASK_DETECTION.faces = np.array(MASK_DETECTION.faces, dtype="float32")
        preds = maskNet.predict(MASK_DETECTION.faces, batch_size=32)
...
maskNet = load_model("C:\Program Files\Innovation projects\CovidEnforcement\MaskCheck\Disease_OutBreak_Assistance\mask_detector.model")
...
while True:
    ...
    MASK_DETECTION(frame, faceNet, maskNet)
    num = "0" if len(MASK_DETECTION.faces) < 1 else "1"
    ...
    for (box, pred) in zip(locs, preds):
        ...
        if textVisual == "Not Safe":
            ...
            img_name = "No_Mask_Person_{}.png".format(TimeStamp)
