...
while True:
    ...
    if config.Thread:
        ...
        (grabbed, Dis_frame) = vs.read()
        # if the frame was not grabbed, then we have reached the end of the stream
        if not grabbed:
            break

    # resize the frame and then detect people (and only people) in it
    Dis_frame = imutils.resize(Dis_frame, width=700)
    ...
    for (i, (prob, bbox, centroid)) in enumerate(Dis_results):
        ...
        if i in serious:
            color = (0, 0, 255)
            playsound(r'C:\Program Files\Innovation projects\CovidEnforcement\MaskCheck\Disease_OutBreak_Assistance\sounds\Distancetestsound.mp3')
        elif i in abnormal:
